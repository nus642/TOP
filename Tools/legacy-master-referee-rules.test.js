...
function reentryHarness(task) {
  const calls = [];
  const fields = new Map();
  const element = id => {
    if (!fields.has(id)) fields.set(id, { value: '', innerText: '', readOnly: false, disabled: false, tagName: id === 'gameFormat' || id === 'scoreRule' || id === 'matchTypeSel' ? 'SELECT' : 'INPUT', setAttribute() {}, classList: { add() {}, remove() {} } });
    return fields.get(id);
  };
  const assignment = { lifecycle: 'in_progress', match_id: 'M-15', court: '2', task, score: { t1: 4, t2: 3 } };
  const context = {
