...
test('login reconciles v6 recovery and otherwise discovers server authority', () => {
  const block = html.slice(html.indexOf('window.handleLogin ='), html.indexOf('window.handleLogout ='));
  assert.ok(block.indexOf('checkAndRestoreBackup(pwd)') < block.indexOf('discoverServerAssignment(pwd)'));
