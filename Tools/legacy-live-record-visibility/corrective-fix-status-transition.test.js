...
// ======================== 备份恢复路径运行时验证 ========================
describe('备份恢复：checkAndRestoreBackup 运行时行为（真实执行函数）', () => {
  const src = readSource('referee.html');

  // 提取 checkAndRestoreBackup 函数体
  const funcStart = 'function checkAndRestoreBackup() {';
  const funcIdx = src.indexOf(funcStart);
  assert.ok(funcIdx >= 0, 'checkAndRestoreBackup 函数必须存在');
  let braceCount = 0;
  let funcEnd = funcIdx;
  for (let k = src.indexOf('{', funcIdx); k < src.length; k++) {
    if (src[k] === '{') braceCount++;
    else if (src[k] === '}') { braceCount--; if (braceCount === 0) { funcEnd = k + 1; break; } }
  }
  const funcCode = src.slice(funcIdx, funcEnd);

  const BACKUP_KEY = 'pickle_referee_backup_v5';

  function makeSandbox(backupData, confirmResult = true) {
    const calls = { updateReferee: [], syncLive: 0, showStep: [], renderGame: 0, clearBackup: 0 };
    const storage = {};
    if (backupData) storage[BACKUP_KEY] = JSON.stringify(backupData);
    ...
    const sandbox = {
      ...
      gameState: {},
      timeoutUsed: {},
      updateRefereeStatus: (status, court) => { calls.updateReferee.push({ status, court }); },
      syncLiveScore: () => { calls.syncLive++; },
      showStep: (n) => { calls.showStep.push(n); },
      renderGame: () => { calls.renderGame++; },
      clearBackup: () => { calls.clearBackup++; },
      $: (id) => ({ innerText: '', classList: { add: () => {}, remove: () => {} } }),
    };
    vm.createContext(sandbox);
    return { sandbox, calls, storage };
  }

  it('step=2 备份恢复：不调用 updateRefereeStatus/syncLiveScore，恢复到设置页', () => {
    const backupData = {
      step: 2,
      currentMatch: { id: '001-01', court: '1', t1Name: 'A队', t2Name: 'B队', type: 'doubles' },
      matchState: { t1Score: 0, t2Score: 0, over: false, currentGame: 1 },
      gameState: { viewBa: false },
      timeoutUsed: { t1: false, t2: false },
    };
    const { sandbox, calls } = makeSandbox(backupData, true);
    vm.runInContext(funcCode + '\ncheckAndRestoreBackup();', sandbox);

    assert.equal(calls.updateReferee.length, 0, 'step=2 恢复不得调用 updateRefereeStatus');
    assert.equal(calls.syncLive, 0, 'step=2 恢复不得调用 syncLiveScore');
    assert.deepEqual(calls.showStep, [2], '必须恢复到 step 2');
    assert.equal(sandbox.currentMatch.id, '001-01', '比赛数据必须恢复');
    assert.equal(sandbox.currentMatch.court, '1', '场地必须恢复');
    assert.equal(sandbox.matchState.t1Score, 0, '比分必须恢复');
  });

  it('step=3 备份恢复：调用 updateRefereeStatus("执裁中", court) 和 syncLiveScore 各 1 次', () => {
    const backupData = {
      step: 3,
      currentMatch: { id: '001-01', court: '2', t1Name: 'A队', t2Name: 'B队', type: 'doubles' },
      matchState: { t1Score: 2, t2Score: 1, over: false, currentGame: 1 },
      gameState: { viewBa: true },
      timeoutUsed: { t1: false, t2: false },
    };
    const { sandbox, calls } = makeSandbox(backupData, true);
    vm.runInContext(funcCode + '\ncheckAndRestoreBackup();', sandbox);

    assert.equal(calls.updateReferee.length, 1, 'step=3 恢复必须调用 updateRefereeStatus 恰好 1 次');
    assert.equal(calls.updateReferee[0].status, '执裁中', '状态必须为"执裁中"');
    assert.equal(calls.updateReferee[0].court, '2', 'court 必须为备份中的场地号');
    assert.equal(calls.syncLive, 1, 'step=3 恢复必须调用 syncLiveScore 恰好 1 次');
    assert.ok(calls.showStep.includes(3), '必须恢复到 step 3');
    assert.equal(calls.renderGame, 1, 'step=3 恢复必须调用 renderGame');
    assert.equal(sandbox.matchState.t1Score, 2, '比分必须恢复（t1）');
    assert.equal(sandbox.matchState.t2Score, 1, '比分必须恢复（t2）');
  });

  it('无备份时不执行任何操作', () => {
    const { sandbox, calls } = makeSandbox(null, true);
    vm.runInContext(funcCode + '\ncheckAndRestoreBackup();', sandbox);

    assert.equal(calls.updateReferee.length, 0, '无备份不得调用 updateRefereeStatus');
    assert.equal(calls.syncLive, 0, '无备份不得调用 syncLiveScore');
    assert.equal(calls.showStep.length, 0, '无备份不得调用 showStep');
  });

  it('用户拒绝恢复时调用 clearBackup 而非恢复状态', () => {
    const backupData = {
      step: 3,
      currentMatch: { id: '001-01', court: '1', t1Name: 'A队', t2Name: 'B队' },
      matchState: { t1Score: 5, t2Score: 3, over: false },
      gameState: {},
      timeoutUsed: {},
    };
    const { sandbox, calls } = makeSandbox(backupData, false); // confirm returns false
    vm.runInContext(funcCode + '\ncheckAndRestoreBackup();', sandbox);

    assert.equal(calls.updateReferee.length, 0, '用户拒绝恢复不得调用 updateRefereeStatus');
    assert.equal(calls.syncLive, 0, '用户拒绝恢复不得调用 syncLiveScore');
    assert.equal(calls.clearBackup, 1, '用户拒绝恢复必须调用 clearBackup');
  });
});
