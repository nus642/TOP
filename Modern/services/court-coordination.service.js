...
function reportCondition(tournamentValue, courtValue, actor, data = {}) {
  ...
  return db.withTransaction(async (connection) => {
    ...
    const matchId = await repository.findPlayingMatch(tournamentId, court, connection);
    ...
    if (data.condition === "available" && disruption) {
      ...
      disruption = await repository.createDisruption({ tournamentId, courtId: court,
        affectedMatchId: data.affectedMatchId || matchId, condition: data.condition, actorId: masterId }, connection);
    }
