...
async function after() {
  ...
  try {
    ...
    checkpoint("failure-observed", { appContainerId: process.env.RESTART_APP_CONTAINER_ID, stoppedExited: observed("RESTART_APP_STOPPED"), httpUnavailable: observed("RESTART_HTTP_UNAVAILABLE") });
    checkpoint("mysql-continuity-during-failure", { dbContainerId: process.env.RESTART_DB_CONTAINER_ID, startedAt: process.env.RESTART_DB_STARTED_AT, sameContainer: observed("RESTART_DB_SAME_CONTAINER"), sameStartedAt: observed("RESTART_DB_SAME_STARTED_AT"), running: observed("RESTART_DB_RUNNING"), healthy: observed("RESTART_DB_HEALTHY"), readOnlyQuerySucceeded: observed("RESTART_DB_READ_ONLY_QUERY_SUCCEEDED"), authoritativeStateUnchanged: observed("RESTART_DB_STATE_UNCHANGED") });
    await waitForApp(); assert.equal(readRuntimeBuildId(), state.buildIdentity); assert.equal(process.env.RESTART_OCI_REVISION, state.buildIdentity); checkpoint("artifact-identity-after-restart");
