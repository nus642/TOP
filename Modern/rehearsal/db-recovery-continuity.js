...
async function resume() {
  ...
  e.recoveredState={snapshot:recovered,exactRecoveryPoint:true}; e.expectedRpoLoss=requireExpectedRpo({recoveryPoint:e.recoveryPoint,recovered,deltaLedger:e.postBackupDelta}); e.operationalContinuation={freshSessions:true,rpBCompleted:true,resourcesReleasedAndReused:true,contECompleted:true}; e.run.phase="complete"; e.summary={passed:true}; atomic(manifestPath,e);
}
(async()=>{ if(phase==="prepare")await prepare(); else if(phase==="bind-delta")await bindDelta(); else if(phase==="record-host")await recordHost(); else if(phase==="resume")await resume(); else if(phase==="assert-restore-gate"){const e=load();assert.equal(e.run.phase,"restore-gate","run is invalid or not ready for restore; use a fresh run ID after explicit reset");assert.ok(e.postBackupDelta?.allObservedDifferencesAccountedFor,"run lacks computed post-backup delta accounting");} else if(phase==="assert-db-identity"){const e=load(),expected=process.argv[4];assert.match(expected||"",/^[0-9a-f]{64}$/);assert.equal(e.databaseIdentity.containerId,expected,"DB container identity changed across recovery gate");} else if(phase==="probe-cookie")process.stdout.write(await session(`db-recovery-probe-${runId}`,"master")); else if(phase==="verify-post-delta"){const e=load();assert.deepEqual(await dbSnapshot(),e.postBackupDelta.snapshot,"POST-DELTA changed after unpause and before restore");} else if(phase==="snapshot")process.stdout.write(`${JSON.stringify(await dbSnapshot())}\n`); else throw new Error("usage: db-recovery-continuity.js prepare|bind-delta|record-host|resume|assert-restore-gate|assert-db-identity|probe-cookie|verify-post-delta|snapshot RUN_ID"); })().catch(e=>{console.error(e.message);process.exitCode=1;});
