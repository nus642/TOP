...
(async()=>{ if(phase==="prepare")await prepare(); else if(phase==="bind-delta")await bindDelta(); else if(phase==="record-host")await recordHost(); else if(phase==="resume")await resume(); else if(phase==="assert-restore-gate")assert.equal(load().run.phase,"restore-gate","run is invalid or not ready for restore; use a fresh run ID after explicit reset"); else if(phase==="verify-post-delta"){const e=load();assert.deepEqual(await dbSnapshot(),e.postBackupDelta.snapshot,"POST-DELTA changed after unpause and before restore");} else if(phase==="snapshot")process.stdout.write(`${JSON.stringify(await dbSnapshot())}\n`); else throw new Error("usage: db-recovery-continuity.js prepare|bind-delta|record-host|resume|assert-restore-gate|verify-post-delta|snapshot RUN_ID"); })().catch(e=>{console.error(e.message);process.exitCode=1;});
