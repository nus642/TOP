...
async function prepare() {
  ...
  const made = await mutation(g,"create","POST","/api/competition",master,fixture.competition); const id=made.json.competition?.id??made.json.id;
  await mutation(g,"schedule","POST",`/api/competition/${id}/schedule/import`,master,fixture.schedule); await mutation(g,"roster","POST",`/api/referee-coordination/${id}/referees/roster`,master,{refereeIds:fixture.referees});
  for (const state of ["registration_open","ready","running"]) await mutation(g,`lifecycle-${state}`,"POST",`/api/competition/${id}/lifecycle/transition`,master,{state});
  await mutation(g,"check-in","POST",`/api/master-workflow/${id}/check-in-all`,master,{});
  ...
  await dispatch(g,master,id,a,refs[0],cid("pre","rp-a-dispatch")); all=await matches(master,id); let current=all.find(x=>x.matchId===a.matchId); await accept(g,cookies[0],id,current,refs[0],cid("pre","rp-a-accept")); await start(g,cookies[0],id,current,refs[0]); await finish(g,master,cookies[0],id,current,refs[0],7,"pre");
  await dispatch(g,master,id,b,refs[1],cid("pre","rp-b-dispatch")); all=await matches(master,id); current=all.find(x=>x.matchId===b.matchId); await accept(g,cookies[1],id,current,refs[1],cid("pre","rp-b-accept")); await start(g,cookies[1],id,current,refs[1]);
  await dispatch(g,master,id,c,refs[2],cid("pre","rp-c-dispatch")); all=await matches(master,id); const states=[a,b,c,d,e].map(x=>status(all.find(y=>y.matchId===x.matchId))); assert.deepEqual(states,["confirmed","playing","assigned","available","available"]);
