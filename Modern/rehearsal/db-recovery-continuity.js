...
const { buildDeltaLedger } = require("./db-recovery/delta-ledger");
const { readRuntimeBuildId, BUILD_ID_FILE } = require("./build-identity");
const { assertRestoreGateEvidence } = require("./db-recovery/phase-one");
...
async function prepare() {
  validateDataSafety(process.env); assert.ok(!fs.existsSync(dir), "RUN_ID already exists; recovery and watchdog-proof IDs must be distinct"); fs.mkdirSync(dir, { recursive: true, mode: 0o700 });
...
async function bindDelta() {
  ...
  evidence.recoveryPointBinding.snapshotBExact=true;
  const gate=new MutationGate(); assert.equal(gate.state,"OPEN");
  evidence.recoveryPointBinding.gateOpenedAt=new Date().toISOString();
  evidence.recoveryPointBinding.reopenedAsNewProcessMutationEpoch=true;
  evidence.run.phase="post-backup-delta-in-progress";
  atomic(manifestPath,evidence);
  const master=await session(`db-recovery-master-${runId}`,"master"), ref=await session(evidence.scenario.resources.rpC.referee,"referee"), id=evidence.scenario.competitionId; let all=await matches(master,id), c=all.find(x=>x.matchId===evidence.scenario.matchIds.rpC);
  ...
  const post=await dbSnapshot(); const declaredFacts=[cid("post","rp-c-accept"),cid("post",`score-${c.matchId}`),cid("post",`confirm-${c.matchId}`),cid("post","delta-d-dispatch")]; evidence.postBackupDelta={snapshot:post,...buildDeltaLedger(evidence.recoveryPoint,post,gate.outcomes,declaredFacts)}; evidence.run.phase="post-backup-delta-complete"; atomic(manifestPath,evidence);
}
...
async function completeFailureProof() {
  const e=load(); assert.equal(e.run.phase,"post-backup-delta-complete");
  const observed=(name)=>{assert.equal(process.env[name],"true",`missing successful host observation ${name}`);return true;};
  const containerId=process.env.FAILURE_DB_CONTAINER_ID, dockerAccessMode=process.env.FAILURE_DOCKER_ACCESS_MODE;
  assert.match(containerId||"",/^[0-9a-f]{64}$/); assert.equal(containerId,e.databaseIdentity.containerId); assert.match(dockerAccessMode||"",/^(direct|sudo-n)$/);
  const records=fs.readFileSync(path.join(dir,"watchdog.jsonl"),"utf8").trim().split("\n").map(JSON.parse);
  assert.equal(records[0]?.event,"armed"); assert.equal(records[0]?.containerId,containerId); assert.equal(records[0]?.dockerAccessMode,dockerAccessMode);
  assert.equal(records.at(-1)?.event,"disarmed"); assert.equal(records.at(-1)?.containerId,containerId);
  assert.deepEqual(await dbSnapshot(),e.postBackupDelta.snapshot,"POST-DELTA changed after unpause and before restore gate");
  e.environment.dockerAccessMode=dockerAccessMode;
  e.failureObserved={pausedExactId:observed("FAILURE_PAUSED_EXACT_ID"),verifyFailedOrTimedOut:observed("FAILURE_VERIFY_FAILED"),authenticatedDbReadSucceededBeforePause:observed("FAILURE_AUTH_READ_BEFORE"),dbBackedReadFailedOrTimedOut:observed("FAILURE_DB_READ_FAILED"),writesIssued:false,postDeltaReverifiedAfterUnpause:true};
  e.watchdog={path:path.join(dir,"watchdog.jsonl"),containerId,dockerAccessMode,armed:true,explicitUnpauseHealthy:observed("FAILURE_UNPAUSE_HEALTHY"),normalPathDisarmed:true};
  e.run.phase="restore-gate"; assertRestoreGateEvidence(e); atomic(manifestPath,e);
}
function completeFinalIntegrity() { const e=load(); assert.equal(e.run.phase,"continuation-complete"); e.finalIntegrity={existingVerifyPassed:true}; e.summary={passed:true}; e.run.phase="complete"; atomic(manifestPath,e); }
async function resume() {
  ...
  e.recoveredState={snapshot:recovered,exactRecoveryPoint:true}; e.expectedRpoLoss=requireExpectedRpo({recoveryPoint:e.recoveryPoint,recovered,deltaLedger:e.postBackupDelta}); e.operationalContinuation={freshSessions:true,rpBCompleted:true,resourcesReleasedAndReused:true,contECompleted:true}; e.run.phase="continuation-complete"; e.summary={passed:false,awaitingFinalIntegrity:true}; atomic(manifestPath,e);
}
(async()=>{ if(phase==="prepare")await prepare(); else if(phase==="bind-delta")await bindDelta(); else if(phase==="record-host")await recordHost(); else if(phase==="complete-failure-proof")await completeFailureProof(); else if(phase==="complete-final-integrity")completeFinalIntegrity(); else if(phase==="resume")await resume(); else if(phase==="assert-restore-gate")assertRestoreGateEvidence(load()); else if(phase==="assert-db-identity"){const e=load(),expected=process.argv[4];assert.match(expected||"",/^[0-9a-f]{64}$/);assert.equal(e.databaseIdentity.containerId,expected,"DB container identity changed across recovery gate");} else if(phase==="probe-cookie")process.stdout.write(await session(`db-recovery-probe-${runId}`,"master")); else if(phase==="verify-post-delta"){const e=load();assert.deepEqual(await dbSnapshot(),e.postBackupDelta.snapshot,"POST-DELTA changed after unpause and before restore");} else if(phase==="snapshot")process.stdout.write(`${JSON.stringify(await dbSnapshot())}\n`); else throw new Error("usage: db-recovery-continuity.js prepare|bind-delta|record-host|complete-failure-proof|complete-final-integrity|resume|assert-restore-gate|assert-db-identity|probe-cookie|verify-post-delta|snapshot RUN_ID"); })().catch(e=>{console.error(e.message);process.exitCode=1;});
