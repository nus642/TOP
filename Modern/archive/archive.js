(function expose(factory) {
  const uiText = typeof module !== "undefined" && module.exports
    ? require("../shell/ui-text")
    : window.UiText;
  const archive = factory(uiText);
  ...
  const statusLabels = Object.freeze({ confirmed: "已确认" });
  ...
  function renderArchive(data) {
    ...
    return {
      summary: `<p class="archive-number">比赛 ${escapeHtml(data.competitionId)}</p><span class="completion-state">${escapeHtml(UiText.competitionLifecycleLabel(data.competitionStatus))}</span>`,
